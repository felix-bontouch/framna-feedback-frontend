...
5. Consider skip logic opportunities with existing fields`
  ...
  async generateFormTheme(theme: string, prompt: string): Promise<any> {
    if (!this.openai) {
      throw new Error('OpenAI API is not configured')
    }

    const systemPrompt = `You are a professional UI/UX designer specializing in form aesthetics and user experience. Generate a beautiful, cohesive color scheme and styling for forms.

Design Principles:
1. Ensure high contrast for accessibility (WCAG AA compliance)
2. Create a cohesive color palette that matches the theme
3. Consider color psychology and emotional impact
4. Ensure readability on all devices
5. Maintain professional appearance

Generate a complete theme configuration including:
- Font family (e.g., Inter, system-ui, sans-serif)
- Question text color (hex format, e.g., #1a1a1a)
- Answer text color (hex format, e.g., #4a4a4a)
- Button background color (hex format)
- Button text color (hex format, e.g., #ffffff)
- Background color (hex format, e.g., #ffffff)
- Optional background image URL
- Background brightness adjustment (0-1)`
    ...
    try {
      const response = await this.openai.chat.completions.create({
        ...
        temperature: 0.1,
        tools: [AI_TOOLS.generateFormTheme],
        tool_choice: { type: 'function', function: { name: 'generate_form_theme' } }
      })
      ...
      const toolCall = message.tool_calls[0]
      if (toolCall.function.name !== 'generate_form_theme') {
        throw new Error('Expected generate_form_theme tool call')
      }

      return JSON.parse(toolCall.function.arguments)
    } catch (error) {
      console.error('Error generating form theme:', error)
      throw new Error('Failed to generate form theme')
    }
  }
